...
class Medal {
  ...
  unlock() {
    ...
    localStorage[
      "OS13kTrophy," + this.icon + "," + medalsSaveName + "," + this.name
    ] = this.description;
  }
  ...
  render(hidePercent = 0) {
    ...
    this.renderIcon(
      x + 15 + medalDisplayIconSize / 2,
      y + medalDisplayHeight / 2
    );
  ...
  renderIcon(x, y, size = medalDisplayIconSize) {
    ...
    context.font = size * 0.7 + "px " + fontDefault;
    if (this.image.src)
      context.drawImage(this.image, x - size / 2, y - size / 2, size, size);
    else context.fillText(this.icon, x, y); // show icon if there is no image
  }
...
class Newgrounds {
  ...
  constructor(app_id, cipher) {
    ...
    if (this.session_id == 0) return; // only use newgrounds when logged in
